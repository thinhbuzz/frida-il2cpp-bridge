...
export class Il2CppObject extends NativeStruct {
    ...
    method<T extends MethodReturnType>(
        name: string,
    ...
    virtualMethod<T extends MethodReturnType>(
        method: Method,
    ): BoundMethod<T> {
        return new Method<T>(
            objectGetVirtualMethod.value(this, method),
        ).bind(this);
    }

    /** Gets the non-static field with the given name of the current class hierarchy, if it exists. */
    tryField<T extends FieldType>(
        name: string,
    ): BoundField<T> | undefined {
